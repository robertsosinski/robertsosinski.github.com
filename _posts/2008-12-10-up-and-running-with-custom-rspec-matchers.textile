...
Once you have that setup, spec it out to see if everything works as it should:
...
That looks a lot better. Notice that you not only have your new matcher available, but you can use your choice of either @be_two@ or @equal_two@.  Obviously, there is no need to have both in a test as they are the same exact matcher. This does help however, when the rules of the English language dictate so (e.g. "the_car.should have_wheels" vs. "the_wheelbarrow.should have_a_wheel").
